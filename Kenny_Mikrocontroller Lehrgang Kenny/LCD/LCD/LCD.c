...
void lcd_integer(uint16_t * integerToSend){
	char buffer[10];
	itoa(integerToSend,buffer,10);
	lcd_string(buffer);
}
void lcd_float(float number, int width, int precision){
	char buffer[20];
	dtostrf(number,width,precision,buffer);
	lcd_string(buffer);	
}
void lcd_gotoxy(int zeile, int spalte){
	if (zeile==0) {
		lcd_command(0x80+spalte);	
	}
	if (zeile==1) {
		lcd_command(0xC0+spalte);
	}
}